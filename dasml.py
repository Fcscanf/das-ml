...
# 创建一个logger
logger = logging.getLogger("dasml")
...
###
    ...
    def isDistanceNearKm(self, km) -> bool:
        pt = km/KM_PER_PT
        return pt >= self.ptRange[0] and pt <= self.ptRange[1]

    def isNearBy(self, timeStamp, pt, type) -> bool:
        if type != self.type:
            return False
        return (timeStamp >= (self.timeRange[0]-timedelta(seconds=EVENT_MAX_TIME_RANGE)) and timeStamp <= self.timeRange[1]) and (pt >= self.ptRange[0] and pt <= self.ptRange[1])
    ...
    def updateLevel(self) -> int:
        ...
        if duration >= EVENT_VISIBLE_TIME and duration < EVENT_SUSPECTED_TIME:
            self.level = 1
        elif duration >= EVENT_SUSPECTED_TIME and duration < EVENT_DANGROUS_TIME:
            self.level = 2
    ...
    def genAlarmPackage(self, startOrStop) -> bytes:
        if startOrStop:
            self.alarmTimes += 1
        self.updated = False
        return f"DVS,{'S' if startOrStop else 'E'},{self.id},1,1,{self.getCenterKm():.3f},{self.level},{self.type},{self.alarmTimes},{self.heat}#".encode(encoding='UTF-8')
...
async def melEngineProc(name, ipaddr):
    ip, port = ipaddr
    reader, writer = await asyncio.open_connection(ip, port)
    logger.info(f'[{name}] Connecting to server {ipaddr} succeed!')

    workDir = os.path.dirname(os.path.abspath(__file__))
    weightFile = f'{workDir}/pretrained_dropout.pkl'
    logger.info(f"[{name}] Loading NP weights from: {weightFile}")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    dasNet = DasNet().to(device)
    dasNet.load_state_dict(torch.load(weightFile, map_location=device))
    dasNet.eval()
    ...
    try:
        while True:
            ...
            with open('outputs.txt', 'a', encoding='utf-8') as outputFile:
                outputFile.write(str(outputs))
                outputFile.write('\n')
            notifierQueue.put(outputs)
    except Exception as ex:
        logger.error(f"[melEngineProc] ERROR: {ex}")
    finally:
        writer.close()
...
async def notifierProc(name, ipaddr):
    ...
    while(True):
        # heartbeat 5s
        newTick = time.time()
        if newTick-tick >= HEART_BEAT_DURATION:
            tick = newTick
        ...
        for pt, type, confidence in result:
            ...
            if type == 10:
                logger.debug(
                    f"[New Event] Type: firberBreak, km: {pt*0.02}, confidence: {confidence}")
        ...
        for event in eventToSend:
            logger.info(
                f'[Sending Alarm] Type: {event.getType}, km: {event.getCenterKm()}, duration: {event.getDuration()}')
...
async def tcp_reconnect(name, ipaddr, proc):
    while True:
        ...
        try:
            ...
            logger.info(
                f'[{name}] Connecting to server {ipaddr} failed! ConnectionRefused.')
        except ConnectionResetError:
            logger.info(
                f'[{name}] Connecting to server {ipaddr} failed! ConnectionReset.')
        except asyncio.TimeoutError:
...
async def main():
    coros = [tcp_reconnect("MelEngine", deviceAddr, melEngineProc),
